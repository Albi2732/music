...
spotify = spotipy.Spotify(client_credentials_manager=SpotifyClientCredentials(client_id='1ff69d94deb24751b4e1a55c9bf9301d',
client_secret='05f6def4895641848da6cfeaa6afceef'))
myPlaylists = []
...
def deletePlaylist(request):
    if request.method=="POST":
        ...
        messages.info(request,"Плейлист удален")
    ...
    try:
        ...
        if password==cpassword:
            if User.objects.filter(username = username).exists():
                ...
                messages.info(request,"Адрес почты уже занят!")
                # print("Email Taken")
                return redirect("/")
            else:
